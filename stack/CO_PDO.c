...
//    if(dataLen&0x07) return CO_SDO_AB_NO_MAP;   /* Object cannot be mapped to the PDO. */
    if(dataLen&0x07) return 1;   /* Object cannot be mapped to the PDO. */
...
//        return CO_SDO_AB_NOT_EXIST;   /* Object does not exist in the object dictionary. */
    ...
    void *object = CO_OD_find(OD, index);
    if (NULL == object)
    {
...
//    attr = CO_OD_getAttribute(&SDO->OD, entryNo, subIndex);
    attr = CO_OD_getAttribute(object, subIndex);
...
//            return CO_SDO_AB_NO_MAP;   /* Object cannot be mapped to the PDO. */
            return 1;   /* Object cannot be mapped to the PDO. */
    }
...
//    objectLen = CO_OD_getLength(&SDO->OD, entryNo, subIndex);
    objectLen = CO_OD_getLength(object, subIndex);
...
//    *ppData = (uint8_t*) CO_OD_getDataPointer(&SDO->OD, entryNo, subIndex);
    *ppData = (uint8_t*) CO_OD_getDataPointer(object, subIndex);
#ifdef CO_BIG_ENDIAN
...
//{
//    /* verify arguments */
//    if(RPDO==NULL || SDO==NULL || operatingState==NULL ||
...
int16_t CO_TPDOsend(CO_TPDO_t *TPDO){
    ...
    TXbuff.ident = *TPDO->TPDOCommPar_ptrs.COB_IDUsedByTPDO + TPDO->nodeId;
    TXbuff.DLC = TPDO->dataLength;


    i = TPDO->dataLength;
    pPDOdataByte = &TXbuff.data[0];
    ...
    return CO_CANsend(TPDO->CANdevTx, &TXbuff);
}
